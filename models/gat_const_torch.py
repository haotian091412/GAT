...
ConstAttnHead = layers_torch.ConstAttnHead


class GATConstTorch(nn.Module):
    def __init__(self, in_sz, nb_classes, nb_nodes, hid_units, n_heads, activation=F.elu, residual=False,
                 attn_drop=0.0, ffd_drop=0.0):
        super(GATConstTorch, self).__init__()
        self.in_sz = in_sz
        ...
        for i in range(len(hid_units)):
            ...
            for _ in range(n_heads[i]):
                layer_heads.append(
                    ConstAttnHead(prev_sz, hid_units[i], activation=activation,
                                  in_drop=ffd_drop, residual=residual if i > 0 else False)
                )
        ...
        for _ in range(n_heads[-1]):
            self.out_heads.append(
                ConstAttnHead(prev_sz, nb_classes, activation=lambda x: x,
                              in_drop=ffd_drop, residual=False)
            )
    ...
    def forward(self, inputs, adj_mat):
        ...
        for i, layer_heads in enumerate(self.layers):
            ...
            for head in layer_heads:
                head_outputs.append(head(x, adj_mat))
            x = torch.cat(head_outputs, dim=-1)

        out = []
        for head in self.out_heads:
            out.append(head(x, adj_mat))
